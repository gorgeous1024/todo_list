...
<div class="space-y-4">
		...
		<div class="relative flex-1">
			...
			{#if searchInput}
				<button
					...
					class="absolute right-3 top-1/2 transform -translate-y-1/2 text-theme-tertiary hover:text-theme-secondary"
				>
					<X size={16} />
				</button>
			{/if}
			...
			<div class="relative">
				...
				>
					<option value="created_at-desc"
						>{$_("filterBar.newestFirst")}</option
					>
					<option value="created_at-asc"
						>{$_("filterBar.oldestFirst")}</option
					>
					...
					<option value="due_date-asc"
						>{$_("filterBar.dueDateNearest")}</option
					>
					<option value="due_date-desc"
						>{$_("filterBar.dueDateFurthest")}</option
					>
					<option value="priority-desc"
						>{$_("filterBar.priorityHighToLow")}</option
					>
					<option value="priority-asc"
						>{$_("filterBar.priorityLowToHigh")}</option
					>
				</select>
				<ArrowUpDown
	...
	{#if showFilters}
		<div
			class="bg-theme-secondary rounded-lg border border-theme p-4 transition-colors duration-200"
		>
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
				<!-- Status Filter -->
				<div>
					<label for="status-filter" class="label"
						>{$_("filterBar.status")}</label
					>
					<select
						id="status-filter"
						value={currentFilter.status || ""}
						on:change={(e) =>
							viewActions.setFilter({
					...
					>
						...
						<option value={TaskStatus.TODO}
							>{$_("taskForm.toDo")}</option
						>
						<option value={TaskStatus.IN_PROGRESS}
							>{$_("taskForm.inProgress")}</option
						>
						<option value={TaskStatus.DONE}
							>{$_("taskForm.done")}</option
						>
					</select>
				</div>
						...
						on:change={(e) =>
							viewActions.setFilter({
								priority: e.currentTarget.value
									? (e.currentTarget.value as TaskPriority)
									: undefined,
							})}
						class="input"
					>
						...
						<option value={TaskPriority.HIGH}
							>{$_("taskForm.highPriority")}</option
						>
						<option value={TaskPriority.MEDIUM}
							>{$_("taskForm.mediumPriority")}</option
						>
						<option value={TaskPriority.LOW}
							>{$_("taskForm.lowPriority")}</option
						>
					</select>
				</div>
				...
				<!-- Completion Filter -->
				<div>
					<label for="completion-filter" class="label"
						>{$_("common.completion")}</label
					>
					<select
						id="completion-filter"
						value={currentFilter.completed !== undefined
							? currentFilter.completed.toString()
							: ""}
						on:change={(e) =>
							viewActions.setFilter({
								completed: e.currentTarget.value
